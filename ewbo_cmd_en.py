...
# (English version — title stays the same)
...
messages = [
    ("Closing every program that fills you with worry...", 1),
    ("Temporarily clearing thoughts that reach too far ahead...", 1),
    ("Rearranging hopes that got a little scattered...", 2),
    ("Saving the good things still worth holding on to...", 1),
    ("Letting go of things whose time has simply passed...", 2),
    ("Processing fears that have been carried for too long...", 3),
    ("Remembering that not everything has to be finished today...", 3),
    ("Moving a weight that was carried alone for too long...", 3),
    ("Making room for a heart that is learning to be calm...", 2),
    ("Restoring a little trust in tomorrow...", 2),
    ("Leaving space for the good things that might still come...", 1),
    ("Almost done... slowly, it's okay.", 2),
]
...
def dialog():
    ...
    input(DIM + "\n[ Press Enter for OK ]" + RESET)
    # clear the "[ Press Enter for OK ]" prompt line after it's pressed
...
def make_a_wish():
    print()
    wish = input(
        WHITE + "If you'd like, write one small wish for today: " + RESET
    ).strip()
    if not wish:
        wish = "something good, whatever that may be"

    # clear the question line + typed answer
    sys.stdout.write("\033[1A" + CLEAR_LINE + "\r")
    sys.stdout.write("\033[1A" + CLEAR_LINE + "\r")
    sys.stdout.flush()

    # brief pause, as if "reading" what was just written
    time.sleep(1.3)

    print()
    type_simple("Keeping your wish safe", delay=0.06, end="")
    for _ in range(3):
        time.sleep(0.35)
    ...
    time.sleep(1.0)
    print(GRAY + "...may it find its way." + RESET)
    time.sleep(0.8)
...
def closing():
    lines = [
        "Done.",
        "Maybe everything isn't okay just yet.",
...
def main():
    ...
    clear()
    print_title()
    print(WHITE + "Press Enter to begin the process." + RESET)
    input()
    print()
    run_loading()
    dialog()
    make_a_wish()
    closing()
    input(DIM + "Press Enter to close this window..." + RESET)
